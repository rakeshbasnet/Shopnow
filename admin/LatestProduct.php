<?php
        ...
        $fileType = pathinfo($target,PATHINFO_EXTENSION);
          if(in_array($fileType, $allowTypes)){
              ...
              /*foreach($_FILES['images']['name'] as $key=>$val){
               ...
               $sql2 = "INSERT INTO Products (PName,CName,Price,Description,Image) VALUES ('$pname','$cname','$price','$des','$image')";
  ...
  if (isset($_POST['delete'])){
    ...
    $result1 = mysqli_query($db, "SELECT Image FROM Products WHERE PName='$pname'");
    $row1 = mysqli_fetch_array($result1);
    $image1=$row1['Image'];
    $file_to_delete1 = 'Products/'.$image1;
    unlink($file_to_delete1);

    $sql = "DELETE FROM Products WHERE PName='$pname' AND CName='$cname'";
    // execute query
    mysqli_query($db, $sql);
...
<form action="LatestProduct.php" method="POST" enctype="multipart/form-data">
  <div class="container">
    
    <label for="cname"><b>Category Name:</b></label><br>
    <input type="text" placeholder="Enter category Name" name="cname" required><br>
